...
OPENERP_MODULE_NAME = 'res.groups'

class ResGroups(OErpModel):

    def __init__(self):
        super(ResGroups, self).__init__()
    ...
    def process(self, wrksht, rowTask):
        super(ResGroups, self).process(wrksht, rowTask)
        
        for idx, aMethod in enumerate(self.methodNames):

            if super(ResGroups, self).todo(idx):
                print '    #{} Doing "{}" now.'.format(idx+1, aMethod)
                super(ResGroups, self).starting(idx)


            	self.methods[aMethod](self.parameters)


                super(ResGroups, self).finished(idx)
                print '__'
            else:
    ...
    def load(self, parms):
        ...
        data = super(ResGroups, self).load(parms, OPENERP_MODULE_NAME)
        print 'Done in ResGroups!'

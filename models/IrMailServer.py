...
class IrMailServer(OErpModel):
    ...
    def update(self, parms):
        ...
        idMailServer = oerp.search(OPENERP_MODULE_NAME, [(identifier, 'ilike', parms[identifier])])[0]
        ...
        for key in parms:
            if key not in ignored_fields:    # Ignore the record key attribute
                ...
                print 'Editing : (Mail server : {}, "{}":"{}") from "{}"'.format(parms[identifier], key, val, parms[key])
